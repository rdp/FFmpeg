...
const CLSID CLSID_BDA_MPEG2_Transport_Informatiuon_Filter =
    {0xFC772AB0,0x0C7F,0x11D3,{0x8F,0xF2,0x00,0xA0,0xC9,0x22,0x4C,0xF4}};
const CLSID CLSID_MS_DTV_DVD_Decoder =
    //{0x3C78B8E2,0x6C4D,0x11D1,{0xAD,0xE2,0x00,0x00,0xF8,0x75,0x4B,0x77}}; //blaze
    //{0xEE30215D,0x164F,0x4A92,{0xA4,0xEB,0x9D,0x4C,0x13,0x39,0x0F,0x9F}};   //lav filter
    {0x212690FB,0x83E5,0x4526,{0x8F,0xD7,0x74,0x47,0x8B,0x79,0x39,0xCD}}; //ms
...
static int
dshow_connect_bda_pins(AVFormatContext *avctx, IBaseFilter *source, const char *src_pin_name, IBaseFilter *destination, 
    const char *dest_pin_name, IPin **ext_pin, const char *ext_pin_name ) // ext is "exiting pin" from the dest...just makes sure it's there [?]
{
    struct dshow_ctx *ctx = avctx->priv_data;
    IEnumPins *pins = 0;
    ...
    if (ext_pin != NULL) {
        ...
        if (!(*ext_pin)) {
            if (ext_pin_name)
                av_log(avctx, AV_LOG_ERROR, "Destination filter doesn't have ext output pin named \"%s\".\n", ext_pin_name);
            else
                av_log(avctx, AV_LOG_ERROR, "Destination filter doesn't have ext output pin.\n");
            return AVERROR(EIO);
        }
...
static int dshow_read_header(AVFormatContext *avctx)
{
    ...
    if (ctx->dtv){
        ...
        //IBaseFilter *ms_dtv_dec = NULL;
        libAVPin *capture_pin = NULL;
        libAVFilter *capture_filter = NULL;
        IScanningTuner *scanning_tuner = NULL;
        //ITuningSpaceContainer *tuning_space_container = NULL;
...
//        //r = CoCreateInstance(__uuidof(SystemTuningSpaces), NULL, CLSCTX_INPROC_SERVER,
//        r = CoCreateInstance(&CLSID_SystemTuningSpaces, NULL, CLSCTX_INPROC_SERVER,
//                             &IID_ITuningSpaceContainer, (void **) &tuning_space_container);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Could not create system tuning spaces: %d.\n", ctx->dtv);
//            goto error;
//        }
//
//
//        r = ITuningSpaceContainer_get_Count(tuning_space_container, &tuning_space_count);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Error counting system tuning spaces.\n");
//            goto error;
//        }
//
//        av_log(avctx, AV_LOG_INFO, "Number of available tuningspaces: %ld.\n", tuning_space_count);
//
//        r = ITuningSpaceContainer_get_EnumTuningSpaces(tuning_space_container, &tuning_space_enum);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Error acquiring tuning space enumerator.\n");
//            goto error;
//        }
//
...
//            char *friendly_name = NULL;
//
//            r = ITuningSpace_get_FriendlyName(tuning_space, &olestr);
//            if (r != S_OK) {
//                av_log(avctx, AV_LOG_ERROR, "Error acquiring tuning space names.\n");
//                goto error;
//            }
//
//            friendly_name = dup_wchar_to_utf8(olestr);
//            av_log(avctx, AV_LOG_INFO, "Tuning Space names: (0x%X) %s <0x%X>.\n", (int) tuning_space, olestr, (int) friendly_name);
//
//            //if (olestr)
...
//
////
//        r = ITuningSpace_CreateTuneRequest(&tune_request);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Could not create tune request\n");
//            goto error;
//        }
//
//        r = ITuneRequest_get_TuningSpace(&tune_request, &tuning_space);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Could not get tuning space\n");
//            goto error;
//        }
////
...
//        r = dshow_get_network_type(ctx->dtv, &CLSIDNetworkType);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Invalid tuningspace.\n");
//            goto error;
//        }
        ...
        while (S_OK == IEnumTuningSpaces_Next(tuning_space_enum, 1, &tuning_space, NULL))
        {
            ...
            if(r != S_OK) {
                ...
                av_log(avctx, AV_LOG_ERROR, "Cannot get UniqueName for Tuning Space: r=0x%8lx\n", r );
                goto error;
            }
        ...
        if (r != S_OK) {
            av_log(avctx, AV_LOG_ERROR, "Error validating dvb tune request: r=0x%8lx\n", r);
            goto error;
        }
...
//            r = IBaseFilter_Release(bda_net_provider);
//            if (r != S_OK) {
//                av_log(avctx, AV_LOG_ERROR, "Could not remove old BDA network provider.\n");
//                goto error;
//            }
        ...
        // dshow_connect_bda_pins(AVFormatContext *avctx, IBaseFilter *source, const char *src_pin_name, 
        // IBaseFilter *destination, const char *dest_pin_name, IPin **ext_pin, const char *ext_pin_name )

        r = dshow_connect_bda_pins(avctx, ctx->device_filter[VideoDevice], NULL, bda_mpeg2_demux, NULL, &bda_src, "3"); // pins configured later [?] https://msdn.microsoft.com/en-us/library/windows/desktop/dd387912(v=vs.85).aspx
        if (r != S_OK) {
        ...
        av_log(avctx, AV_LOG_INFO, "Using bda tuning stuff %d\n", ctx->dtv);
        ...
        if ((ctx->dtv > 0) && (ctx->dtv < 4)) {
            ...
            if (ctx->dtv==1){
                ...
                r = IDVBTuneRequest_put_Locator(dvb_tune_request, dvbc_locator);
                if (r != S_OK) {
                    av_log(avctx, AV_LOG_ERROR, "Could not set DVB-C Locator to DVBTuneRequest\n");
                    goto error;
                }

                r = IScanningTuner_Validate(scanning_tuner, dvb_tune_request);
                if (r != S_OK) {
                    av_log(avctx, AV_LOG_ERROR, "Error validating dvb tune request: r=0x%8lx\n", r );
                    goto error;
                }

                r = IScanningTuner_put_TuneRequest(scanning_tuner, dvb_tune_request);
                if (r != S_OK) {
                    av_log(avctx, AV_LOG_ERROR, "Could not set dvb tune request\n");
                    goto error;
                }
            ...
            if (ctx->dtv==2){
                ...
                if (ctx->tune_freq>0){
                    av_log(avctx, AV_LOG_INFO, "Set frequency %ld\n", ctx->tune_freq);
                    r = IDVBCLocator_put_CarrierFrequency(dvbt_locator, ctx->tune_freq );
                    if (r != S_OK) {
                ...
                r = IDVBTuneRequest_put_Locator(dvb_tune_request, dvbt_locator);
                if (r != S_OK) {
            ...
            if (ctx->dtv==3){
                av_log(avctx, AV_LOG_ERROR, "Currently not present\n");
                goto error;
            }
        ...
        if (ctx->dtv == 4) {
            ...
            av_log(avctx, AV_LOG_INFO, "ATSC Tune Request acquired.\n");
            ...
            if (ctx->tune_freq>0){
                av_log(avctx, AV_LOG_INFO, "Set ATSC frequency %ld\n", ctx->tune_freq);
            ...
            r = IATSCChannelTuneRequest_put_Locator(atsc_tune_request, atsc_locator);
            if (r != S_OK) {
                av_log(avctx, AV_LOG_ERROR, "Could not set ATSC Locator to ATSCTuneRequest\n");
                goto error;
            }

            r = IScanningTuner_Validate(scanning_tuner, atsc_tune_request);
            if (r != S_OK) {
                av_log(avctx, AV_LOG_ERROR, "Error validating atsc tune request: r=0x%x\n", r );
                goto error;
            }

            r = IScanningTuner_put_TuneRequest(scanning_tuner, atsc_tune_request);
            if (r != S_OK) {
...
//        r = IGraphBuilder_Connect(graph, bda_src, capture_pin);
//        if (r != S_OK) {
//            av_log(avctx, AV_LOG_ERROR, "Could not connect capture pins.\n");
//            goto error;
//        }
//
        ...
        if (ctx->dtv_graph_file) { // serialize graph to file
            const WCHAR wszStreamName[] = L"ActiveMovieGraph";
            IStorage *p_storage = NULL;
            IStream *ofile_stream = NULL;
            IPersistStream *pers_stream = NULL;
            //const WCHAR *gfilename = L"de_graph001.grf";
            ...
            //gfilename = (WCHAR*) ctx->dtv_graph_file;

            //r = SHCreateStreamOnFile ((LPCSTR) ctx->dtv_graph_file, STGM_CREATE | STGM_READWRITE, &ofile_stream);
            ...
            //if (SUCCEEDED(r)) {
                r = IStorage_Commit(p_storage, STGC_DEFAULT);
            //}
//            if (r != S_OK) {
...
//            }
        ...
        ////need a properties for demux too....



        //r = dshow_open_dtv(avctx);
//        r= -1;
//        goto error;
    }
